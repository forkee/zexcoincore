...
const char * const BITCOIN_CONF_FILENAME = "zexcoin.conf";
const char * const BITCOIN_PID_FILENAME = "zexcoind.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "zexcoin" is a composite category enabling all ZexCoin-related debug output
            if(ptrCategory->count(string("zexcoin"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\ZexCoinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\ZexCoinCore
    // Mac: ~/Library/Application Support/ZexCoinCore
    // Unix: ~/.zexcoincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ZexCoinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/ZexCoinCore";
#else
    // Unix
    return pathRet / ".zexcoincore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty zexcoin.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override zexcoin.conf
